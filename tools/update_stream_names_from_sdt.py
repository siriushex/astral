...
async def main_async() -> int:
    ...
    # Pre-filter list (fast) before acquiring concurrency slots.
    ...
    eprint(f"Streams selected: {len(work)} (dry_run={dry_run}, parallel={args.parallel}, timeout={args.timeout_sec}s)")
    ...
    async def handle_stream(item: Dict[str, Any]) -> None:
        ...
        # Prefer the first input's pnr if present.
        preferred_sid = None
