...
function ai_openai_client.request_json_schema(opts, callback)
    ...
    local function perform_request()
        ...
        if scrubbed > 0 then
            -- Keep this detail for UI/debug, but do not log the body (it may contain sensitive config info).
            -- `meta.scrubbed_control_bytes` is populated later, when we have `meta`.
        end
